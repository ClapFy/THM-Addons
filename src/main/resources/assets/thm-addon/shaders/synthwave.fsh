#version 330 core
// Adapted from Shadertoy ("another synthwave sunset thing") for THM Addons
...
#define iMouse mouse
// ponytail: no per-frame delta in ThmShaderData; only used to jitter the motion-blur
// sample time, so a fixed 60fps step is indistinguishable here.
...
vec2 intersect(vec3 ro,vec3 rd){
    float d =0.,h=0.;
    for(int i = 0;i<128;i++){ //look nice with 50 iterations (original: 500 — ponytail: cut, see header)
        vec3 p = ro+d*rd;
        vec2 s = map(p);
        h = s.x;
        d+= h*.5;
