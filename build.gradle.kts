...
val generateAPIUtils by tasks.registering {
    ...
    doLast {
        ...
        val payloads = listOf(password, encA1, encA2, encA3, encA4, encA5, encA6, encA7).map { encodePayload(it) }
        ...
        val PF  = Array(8) { Pair(rn(), rn()) }          // (frag0, frag1) per payload
        ...
        val MACI = rn() // cape index URL accessor
        val MTK  = rn() // player's API token accessor
        ...
        // Every human-readable literal that tells an analyst what the class is doing —
        // "AES", "SHA-256", "http://", "Authorization", "Bearer ", the log strings — is routed
        // through the SAME key-reconstruction/fragment/4-path decode as the URLs. Nothing
        // sensitive survives as a constant string, so a decompile-and-grep for "http"/"Cipher"/
        // "Bearer"/log text finds nothing, and recovering any of them costs the full key rebuild.
        // Reuses $MSEL wholesale — no second decode scheme to reverse.
        val litFields = mutableListOf<String>()
        val litCache = HashMap<String, String>()
        fun lit(s: String): String = litCache.getOrPut(s) {
            val enc  = encodePayload(s)
            val mask = IntArray(enc.size) { rng.nextInt(256) }
            val n0 = rn(); val n1 = rn()
            litFields += "    private static final byte[] $n0={${ia(IntArray(enc.size) { enc[it] xor mask[it] })}};"
            litFields += "    private static final byte[] $n1={${ia(mask)}};"
            "$MSEL($n0,$n1)"
        }
        ...
        outputFile.writeText(buildString {
            ...
            appendLine("    public static void sendStatus(String message){if(!xyz.thm.addon.utils.PrivacyGuard.allowsRemoteExport())return;if(xyz.thm.addon.utils.PrivacyGuard.containsCrackedPassword(message))return;$MSA(message,getPassword(),$MAS(),\"status\");}")
            appendLine("    public static void sendStatistics(String message){if(!xyz.thm.addon.utils.PrivacyGuard.allowsRemoteExport())return;if(xyz.thm.addon.utils.PrivacyGuard.containsCrackedPassword(message))return;$MSA(message,getPassword(),$MAH(),\"statistics\");}")
            appendLine("    public static void sendToWebhook(String url,String message){if(!xyz.thm.addon.utils.PrivacyGuard.allowsRemoteExport())return;if(xyz.thm.addon.utils.PrivacyGuard.containsSecrets(message))return;$MSW(url,message);}")
            appendLine("""    public static java.util.List<ThmMembers.Member> fetchMembersFromApi(){try{String response=$MFAR($MAMH());if(response==null)return null;xyz.thm.addon.THMAddon.LOG.info(${lit("Fetched Members")});com.google.gson.Gson gson=new com.google.gson.Gson();com.google.gson.JsonArray jsonArray=gson.fromJson(response,com.google.gson.JsonArray.class);java.util.List<ThmMembers.Member> members=new java.util.ArrayList<>();for(int i=0;i<jsonArray.size();i++){com.google.gson.JsonObject jsonObject=jsonArray.get(i).getAsJsonObject();com.google.gson.JsonArray usernamesArray=jsonObject.getAsJsonArray("usernames");String[]usernames=new String[usernamesArray.size()];for(int j=0;j<usernamesArray.size();j++)usernames[j]=usernamesArray.get(j).getAsString();String rank=jsonObject.get("rank").getAsString();String rankId=jsonObject.has("rankId")?jsonObject.getAsJsonPrimitive("rankId").getAsString():"";String branch=jsonObject.has("branch")?jsonObject.getAsJsonPrimitive("branch").getAsString():"";String discordName=jsonObject.has("discordname")?jsonObject.getAsJsonPrimitive("discordname").getAsString():"";String displayName=usernames.length>0?usernames[0]:"Unknown";if(discordName.isEmpty())discordName=displayName;members.add(new ThmMembers.Member(displayName,usernames,rank,rankId,branch,discordName));}return members;}catch(Exception e){xyz.thm.addon.THMAddon.LOG.warn(${lit("Error fetching members from API: {}")},e.getMessage());}return null;}""")
            appendLine("""    public static java.util.Map<String,String> fetchHighwayStatusFromApi(){try{String body=$MFAR($MAHS());if(body==null)return null;com.google.gson.Gson gson=new com.google.gson.Gson();com.google.gson.JsonObject root=gson.fromJson(body,com.google.gson.JsonObject.class);java.util.Map<String,String> highwayByName=new java.util.HashMap<>();java.util.Map<String,Long> newestTimestampByName=new java.util.HashMap<>();if(root!=null){for(java.util.Map.Entry<String,com.google.gson.JsonElement> entry:root.entrySet()){String highway=entry.getKey();com.google.gson.JsonObject value=entry.getValue().getAsJsonObject();if(value==null||!value.has("username"))continue;String raw=value.get("username").getAsString().trim().toLowerCase(java.util.Locale.ROOT);if(raw.isEmpty()||"unknown".equals(raw))continue;long timestamp=value.has("timestamp")?value.get("timestamp").getAsLong():Long.MIN_VALUE;Long existingTimestamp=newestTimestampByName.get(raw);if(existingTimestamp==null||timestamp>=existingTimestamp){newestTimestampByName.put(raw,timestamp);highwayByName.put(raw,highway);}}}return highwayByName;}catch(Exception e){xyz.thm.addon.THMAddon.LOG.warn(${lit("Error fetching highway status from API: {}")},e.getMessage());return null;}}""")
            appendLine("""    public static java.util.Map<String,String> fetchCapeListFromApi(){try{String body=$MFAR($MACL());if(body==null)return null;com.google.gson.Gson gson=new com.google.gson.Gson();com.google.gson.JsonObject root=gson.fromJson(body,com.google.gson.JsonObject.class);if(root==null||!root.has("players"))return null;com.google.gson.JsonObject players=root.getAsJsonObject("players");java.util.Map<String,String> result=new java.util.HashMap<>();for(java.util.Map.Entry<String,com.google.gson.JsonElement> entry:players.entrySet()){String key=entry.getKey().toLowerCase(java.util.Locale.ROOT);com.google.gson.JsonObject p=entry.getValue().getAsJsonObject();if(p!=null&&p.has("cape"))result.put(key,p.get("cape").getAsString());}xyz.thm.addon.THMAddon.LOG.info(${lit("Fetched Cape List")});return result;}catch(Exception e){xyz.thm.addon.THMAddon.LOG.warn(${lit("Error fetching cape list: {}")},e.getMessage());return null;}}""")
            appendLine("""    public static void postCapeSelection(String username,String cape,String token){new Thread(()->{java.net.HttpURLConnection cn=null;try{java.lang.String url=$MDA($MACP(),getPassword());if(url==null)return;cn=(java.net.HttpURLConnection)new java.net.URI(url).toURL().openConnection();cn.setRequestMethod(${lit("POST")});cn.setRequestProperty(${lit("Content-Type")},${lit("application/json")});if(!token.isEmpty())cn.setRequestProperty(${lit("Authorization")},${lit("Bearer ")}+token);cn.setDoOutput(true);cn.setConnectTimeout(10000);cn.setReadTimeout(10000);java.lang.String json="{\"username\":\""+username.replace("\"","\\\"")+ "\",\"cape\":\""+cape.replace("\"","\\\"")+ "\",\"timestamp\":"+System.currentTimeMillis()+ ",\"token\":\""+token.replace("\"","\\\"")+ "\"}";try(java.io.OutputStream os=cn.getOutputStream()){byte[]b=json.getBytes(java.nio.charset.StandardCharsets.UTF_8);os.write(b,0,b.length);os.flush();}int rc=cn.getResponseCode();try(java.io.InputStream is=rc>=400?cn.getErrorStream():cn.getInputStream()){if(is!=null)is.readAllBytes();}if(rc!=200&&rc!=204)xyz.thm.addon.THMAddon.LOG.warn(${lit("Cape POST response: ")}+rc);}catch(Exception e){xyz.thm.addon.THMAddon.LOG.warn(${lit("Failed to post cape selection")});}finally{if(cn!=null)cn.disconnect();}}).start();}""")
            appendLine("""    public static java.util.List<xyz.thm.addon.utils.CapeManager.CapeEntry> fetchCapeIndexFromApi(){try{String body=$MFAR($MACI());if(body==null)return null;com.google.gson.Gson gson=new com.google.gson.Gson();com.google.gson.JsonObject root=gson.fromJson(body,com.google.gson.JsonObject.class);if(root==null||!root.has("capes"))return null;com.google.gson.JsonArray arr=root.getAsJsonArray("capes");java.util.List<xyz.thm.addon.utils.CapeManager.CapeEntry> result=new java.util.ArrayList<>();for(int i=0;i<arr.size();i++){com.google.gson.JsonObject o=arr.get(i).getAsJsonObject();if(o==null||!o.has("id")||!o.has("url"))continue;result.add(new xyz.thm.addon.utils.CapeManager.CapeEntry(o.get("id").getAsString(),o.get("url").getAsString()));}return result;}catch(Exception e){xyz.thm.addon.THMAddon.LOG.warn(${lit("Error fetching cape index from API: {}")},e.getMessage());return null;}}""")
            litFields.forEach { appendLine(it) }
            appendLine("}")
        }, Charsets.UTF_8)
